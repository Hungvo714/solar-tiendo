...
export default function ReportPage() {
  ...
  function getSchedStatus(item: Item) {
    ...
    const grp  = (item as any).group_type ?? 'B'

    // N/A
    const allNA = steps.length > 0 && steps.every((s: any) => !!progressMap[s.id]?.is_na)
    if (allNA) return { label:'➖ N/A', color:'#8899bb' }

    const planEnd   = g?.plan_end   ? new Date(g.plan_end)   : null
    const planStart = g?.plan_start ? new Date(g.plan_start) : null
    if (planEnd) planEnd.setHours(0,0,0,0)
    if (planStart) planStart.setHours(0,0,0,0)

    const isLate = planEnd && now > planEnd
    const remain = planEnd ? Math.round((planEnd.getTime() - now.getTime()) / 86400000) : null
    const lateN  = planEnd ? Math.round((now.getTime() - planEnd.getTime()) / 86400000) : null

    if (grp === 'A') {
      // Nhóm A - Vật tư
      if (pct >= 1) return { label:'✅ Đã có VT', color:'#4ade80' }
      if (pct === 0) {
        if (isLate) return { label:`🔴 Trễ ${lateN}N`, color:'#ff8888' }
        return { label: remain !== null ? `□ Chưa đặt · Còn ${remain}N` : '□ Chưa đặt', color:'#8899bb' }
      }
      // 0 < pct < 1
      if (isLate) return { label:`🔴 Trễ ${lateN}N`, color:'#ff8888' }
      return { label: remain !== null ? `📦 Đang đặt · Còn ${remain}N` : '📦 Đang đặt', color:'#fbbf24' }
    } else {
      // Nhóm B/C - Thi công
      if (pct >= 1) return { label:'✅ Xong', color:'#4ade80' }
      if (pct === 0) {
        if (isLate) return { label:`🔴 Trễ ${lateN}N`, color:'#ff8888' }
        return { label: remain !== null ? `□ Chưa TC · Còn ${remain}N` : '□ Chưa TC', color:'#8899bb' }
      }
      // 0 < pct < 1
      if (isLate) return { label:`🔴 Trễ ${lateN}N`, color:'#ff8888' }
      return { label:'🔄 Đang TC', color:'#60a5fa' }
    }
  }
